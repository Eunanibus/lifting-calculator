...
export type Settings = { includeBar: boolean; rounding: Rounding; bar: BarId; theme: Theme };

export const DEFAULT_SETTINGS: Settings = { includeBar: true, rounding: 'over', bar: 'barbell', theme: 'dark' };
...
export function loadSettings(storage: StorageLike | undefined = defaultStorage()): Settings {
  try {
    ...
    return {
      includeBar: typeof record.includeBar === 'boolean' ? record.includeBar : DEFAULT_SETTINGS.includeBar,
      rounding: isRounding(record.rounding) ? record.rounding : DEFAULT_SETTINGS.rounding,
